...
datapipe context <config> <query>     Build token-budgeted AI context window
datapipe ui      <config> [-p PORT]   Launch interactive Web UI Visualizer
datapipe mcp     <config>             Launch Model Context Protocol (MCP) server
...
def cmd_context(args) -> None:
    ...
    ctx = builder.build_context(query, max_tokens=args.max_tokens, session_key=args.session)
    
    if args.json:
        print(json.dumps(ctx, indent=2))
    else:
        print(f"[datapipe] Token-Budgeted Context Block for: {query!r}")
        print(f"Tokens Used: {ctx['used_tokens']} / {ctx['max_tokens']} (Token Savings: {ctx['token_savings_pct']}%)\n")
...
def cmd_ui(args) -> None:
    ...
    launch_webui(pipe, port=args.port, open_browser=not args.no_browser)
...
def main() -> None:
    ...
    p = sub.add_parser("context", help="Build token-budgeted context block for LLM prompt")
    p.add_argument("config")
    p.add_argument("query", nargs="+")
    p.add_argument("-m", "--max-tokens", type=int, default=4000)
    p.add_argument("-s", "--session", help="Session memory key")
    p.add_argument("--json", action="store_true", help="Output raw JSON")
    p.set_defaults(func=cmd_context)

    p = sub.add_parser("ui", help="Launch interactive Web UI Visualizer dashboard")
    p.add_argument("config")
    p.add_argument("-p", "--port", type=int, default=8500)
    p.add_argument("--no-browser", action="store_true")
    p.set_defaults(func=cmd_ui)

    p = sub.add_parser("mcp", help="Launch Model Context Protocol (MCP) server over stdio")
    p.add_argument("config")

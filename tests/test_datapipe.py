...
class TestContextBuilder:
    def test_build_context_formats_xml(self, store, tmp_path):
        f = tmp_path / "data.csv"
        f.write_text("title,body\nPython,Learn Python programming\n")
        pipe = Pipeline("ctx", store).source(tmp_path, ["*.csv"]).transform(CSVParser()).columns(["title", "body"])
        pipe.run()

        from datapipe.context import ContextBuilder
        builder = ContextBuilder(pipe)
        ctx = builder.build_context("Python", max_tokens=1000)

        assert "<context_query>Python</context_query>" in ctx["formatted_text"]
        assert "Learn Python programming" in ctx["formatted_text"]
        assert ctx["used_tokens"] > 0
        assert ctx["token_savings_pct"] >= 0.0

    def test_context_with_session_memory(self, store, tmp_path):
        f = tmp_path / "data.csv"
        f.write_text("title,body\nSQLite,SQLite WAL mode\n")
        pipe = Pipeline("ctx_sess", store).source(tmp_path, ["*.csv"]).transform(CSVParser()).columns(["title", "body"])
        pipe.run()

        mem = SessionMemory(store)
        mem.ensure_session("sess-1")
        mem.log_event("sess-1", "search", {"q": "SQLite"}, duration_ms=5)

        from datapipe.context import ContextBuilder
        builder = ContextBuilder(pipe)
        ctx = builder.build_context("SQLite", max_tokens=1000, session_key="sess-1")
        assert "<session_memory>" in ctx["formatted_text"]
        assert "sess-1" in ctx["formatted_text"]


class TestMCPServer:
    def test_mcp_initialize(self, store, tmp_path):
        pipe = Pipeline("mcp_test", store)
        from datapipe.mcp import MCPServer
        server = MCPServer(pipe)

        res = server.handle_request({"id": 1, "method": "initialize"})
        assert res["result"]["serverInfo"]["name"] == "datapipe-mcp"

    def test_mcp_tools_list(self, store, tmp_path):
        pipe = Pipeline("mcp_test", store)
        from datapipe.mcp import MCPServer
        server = MCPServer(pipe)

        res = server.handle_request({"id": 2, "method": "tools/list"})
        tools = [t["name"] for t in res["result"]["tools"]]
        assert "datapipe_search" in tools
        assert "datapipe_get_context" in tools
        assert "datapipe_session_snapshot" in tools
        assert "datapipe_sql" in tools

    def test_mcp_tool_call_search(self, store, tmp_path):
        f = tmp_path / "doc.txt"
        f.write_text("DataPipe context engineering for AI models")
        pipe = Pipeline("mcp_search", store).source(tmp_path, ["*.txt"]).transform(TextParser()).columns(["text"])
        pipe.run()
        ...
        res = server.handle_request({
            "id": 3,
            "method": "tools/call",
            "params": {"name": "datapipe_search", "arguments": {"query": "context"}}
        })
